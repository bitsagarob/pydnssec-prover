...
For every proof in the corpus the port must produce exactly what dnssec-prover 0.6.10 produces:
the same accept or reject decision, the same error, the same valid_from / expires /
max_cache_ttl, the same verified records byte for byte, the same resolve_name output.

Nothing here is compared against the wall clock. The corpus is a set of snapshots and most of it
has expired. Neither implementation checks the clock inside verify_rr_stream; both return the
signature window and leave that to the caller. So each case is judged at a time pinned inside its
own recorded window, the harness applies the wall-clock check itself at that time to both sides,
and staleness relative to now is reported as a note rather than a failure.
"""
...
def materialize_rev(rev: str) -> str:
    ...
    A recorded baseline has to name a commit, since the working tree moves under it.
    """
    import tarfile
    import io
...
def ensure_oracle(autobuild: bool = True) -> str:
    ...
    # Online by default: a fresh cargo cache has none of the locked dependencies, so --offline
    # fails on a clean runner. Set DIFFTEST_OFFLINE=1 once they are cached.
    cmd = [cargo, "build", "--release"]
    if os.environ.get("DIFFTEST_OFFLINE") == "1":
        cmd.append("--offline")
...
def run_python(case_id: str, proof: bytes, name: Optional[str], pinned_time: int) -> Dict[str, Any]:
    ...
    The port does not read the clock today. Pinning anyway keeps a future internal clock check
    from turning this suite red purely because the corpus aged.
    """
    import pyside
...
def _compare_rr_lists(label: str, a: Any, b: Any) -> List[str]:
    """Compare a verified-record list, separating content differences from order differences"""
...
def compare(rust: Dict[str, Any], py: Dict[str, Any]) -> List[str]:
    """Return a list of divergences; an empty list means the two sides agree exactly"""
    diffs: List[str] = []
...
def accepted_at(side: Dict[str, Any], pinned: int) -> bool:
    """The wall-clock layer both implementations leave to the caller"""
    ...
    if rust.get("result") == "valid":
        ...
        if now > int(rust["expires"]):
            notes.append(
                ...
                "snapshot" % ((now - int(rust["expires"])) / 86400.0)
            )
...
def expected_dnssec_accept(case: Case) -> Optional[bool]:
    ...
    expected=invalid with invalid_at_layer=bip353 means the chain is sound and a chain validator
    is expected to accept; the corpus separates that from a DNSSEC rejection on purpose.
    """
