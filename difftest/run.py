...
def print_report(data: Dict[str, Any], quiet: bool = False) -> None:
    ...
    if stale:
        print()
        print(
            "%d of %d proofs are expired in wall-clock terms. That is expected for snapshots and "
            "is NOT a finding; every case above was judged at its pinned time." % (len(stale), len(rows))
        )
